<%-- 
    Document   : Mostrar_Docentes
    Created on : oct 28, 2022, 10:53:13 p.m.
    Author     : kimbe
--%>
...
<div class="loading"></div>
    ...
    <div class="content-page-container full-reset custom-scroll-containers">
		...
		<div class="container-fluid">
            <ul class="nav nav-tabs nav-justified"  style="font-size: 17px;">
              ...
              <li role="presentation" class="active"><a href="#">DOCENTES</a></li>
		 ...
		 <div class="container-fluid"  style="margin: 50px 0;">
            <div class="row">
                ...
                <div class="col-xs-12 col-sm-8 col-md-8 text-justify lead">
                    Bienvenido a la sección para registrar un nuevo Docentes, debes de llenar todos los campos del siguiente formulario para poder registrarlo.
                </div>
            </div>
        </div>
		...
		<div class="row">
			<div class="col-xs-12 lead">
				<ol class="breadcrumb">
				 ...
				 <a href="${pageContext.request.contextPath}/Vista/Admin/Registrar_Docentes.jsp  ">Nuevo Docente</a>
	...
	<div class="container-fluid">
            <h2 class="text-center all-tittles">Listado de Docente</h2>
		  ...
		  <input id="FiltrarContenido" type="text" class="form-control" placeholder="Buscar alumnos..." aria-label="Alumnos" aria-describedby="basic-addon1"> 
            ...
            <div class="div-table">
             <table class="table table-striped">
                        ...
                        Iterator<DOCENTE> iterator = DOCENTEList.iterator();
                        ...
                        while (iterator.hasNext()) {
                            ...
                            DOC.count++;
                    %> 
                        <form name="form-edit" action=".." method="post"> <tr>
                               <th scope="row"><%= DOC.count%></th>
                              <td> <%=DOC.getID_DOCENT()%></td>
                              <td><%=DOC.getDNI_DOCE()%></td>                             
                                 <td><%=DOC.getNOMBRE_DOCEN()%> <%=DOC.getAPELLIDO_DOCEN()%></td>	
                                 <td><%=DOC.getID_RolD()%></td>
                                 <td><%=DOC.getCORREO_DOCEN()%></td>
                                  <td><%=DOC.getTELEFONO_DOCEN()%></td>
                                   <td><%=DOC.getFECHA_DOCEN()%></td>
                                 
                                     <td>   
                                     <% 
                                         ///AQUI HACEMO UN CONDICIONAL DEPENDE EL ESTADO DEL ALUMNO
                                         String estado="";
                                         estado=""+DOC.getESTADO_DOCEN(); 
   if  (estado.equalsIgnoreCase("ACTIVO")) {   %>
                                          
                                                        
                                                        <form  action="post" action="${pageContext.request.contextPath}/ControladorD" > 
                                                            
<a href="${pageContext.request.contextPath}/ControladorD?accion=CambiarEstadoDOCA&idd=<%=DOC.getID_DOCENT()%>"type="submit" class="btn btn-success btn-xs ">Activo</a>
						  ...
						  <form  method="post" action="${pageContext.request.contextPath}/ControladorD" > 
 <a href="${pageContext.request.contextPath}/ControladorD?accion=CambiarEstadoDOCI&idd=<%=DOC.getID_DOCENT()%>"type="submit" class="btn btn-danger btn-xs" >Inactivo</a>
			...
			<a href="<%=DOC.getID_DOCENT()%>" title="Ver datos" data-backdrop="false" class="btn btn-primary btn-sm" data-toggle="modal"><span class="glyphicon glyphicon-eye-open" aria-hidden="true"></span></a>
			
                        
			 
                <a  href="${pageContext.request.contextPath}/ControladorD?accion=Borrar&id=<%=DOC.getID_DOCENT()%>"  title="Eliminar" data-backdrop="false" class="btn btn-danger btn-sm" data-toggle="modal"><span class="glyphicon glyphicon-trash"></span></a>
                       

					 
				<a href="${pageContext.request.contextPath}/Vista/Admin/ActualizarPassDOC.jsp?idpass=<%=DOC.getID_DOCENT()%>&correo=<%=DOC.getCORREO_DOCEN()%>" title="Password" data-backdrop="false" class="btn btn-warning btn-sm" data-toggle="modal"><span class="glyphicon glyphicon-lock"></span></a>
